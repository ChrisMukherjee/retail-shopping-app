...
**Mobile focus:** The testing infrastructure is in place (Jest, `jest-expo`, React Native Testing Library) but mobile tests were not written within the time budget. The highest-value targets would have been: store actions (`useCartStore`, `useCheckoutStore`), the API service layer (Axios mocked), and UI component tests for `DiscountBadge` and `StockBadge`. In a production context these would be the first tests added.
