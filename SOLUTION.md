...
**Mobile focus:** Store actions (`useCartStore`, `useCheckoutStore`), API service layer (`cartService`, `productsService` with Axios mocked), and component tests for `DiscountBadge` and `StockBadge`. Cart store tests specifically cover the expiry/not-found handling across all mutation methods.
